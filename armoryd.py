...
#
# This is a json-rpc interface to armory - http://peercoinarmory.com/
#
...
# application. Requires peercoind process to be running before starting armoryd.
...
# https://peercointalk.org/index.php?topic=92496.0
#####
...
# - The code sometimes returns "peercoinrpc_jsonrpc.authproxy.JSONRPCException"
...
from armoryengine.PyPPCWalletRecovery import *
from inspect import *
from jasvet import readSigBlock, verifySignature
from CppBlockUtils import PPCWallet
...
class InvalidPeercoinAddress(Exception): pass
...
def onlineModeIsPossible(internetAvail, forceOnline):
   ...
   if not forceOnline:
      ...
      LOGINFO('Peercoin-Qt/peercoind is Available: %s', satoshiIsAvailableResult)
...
def addMultWallets(inWltPaths, inWltMap, inWltIDSet):
   ...
   for aWlt in inWltPaths:
      ...
      try:
         wltLoad = PyPPCWallet().readWalletFile(aWlt)
...
class Armory_Json_Rpc_Server(jsonrpc.JSONRPC):
   ...
   def jsonrpc_receivedfromsigner(self, *sigBlock):
      ...
      a Peercoin address and get the amount of coins sent to the current wallet
   ...
   # formatting. Messages are the same type as those generated by Peercoin Core.
   # python armoryd.py verifysignature \"-----BEGIN BITCOIN SIGNED MESSAGE-----$'\n'Comment: Hello.$'\n'-----BEGIN BITCOIN SIGNATURE-----$'\n'$'\n'junkjunkjunk$'\n'-----END BITCOIN SIGNATURE-----\"
   @catchErrsForJSON
   def jsonrpc_verifysignature(self, *sigBlock):
      """
      DESCRIPTION:
      Take a message (RFC 2440: clearsign or Base64) signed by a Peercoin address
   ...
   def jsonrpc_receivedfromaddress(self, sender):
      ...
      RETURN:
      Number of Peercoins sent by the sender to the current wallet.
      """
      totalReceived = 0.0
   ...
   def jsonrpc_listunspent(self):
      ...
      wallet. The dictionary is similar to the one returned by the peercoind
   ...
   def jsonrpc_listaddrunspent(self, inB58):
      ...
      for addrStr in addrList:
         ...
         if not TheBDM.scrAddrIsRegistered(addrStr_to_scrAddr(addrStr)):
            raise PeercoindError('Address is not registered, requires rescan')
         ...
         if atype==ADDRBYTE:
            ...
            if not lbox:
               raise PeercoindError('Import lockbox before getting P2SH unspent')
   ...
   def jsonrpc_importprivkey(self, privKey):
      ...
      privKey - A private key in any format supported by Armory, including
                Base58 private keys supported by peercoind (uncompressed public
                key support only).
      RETURN:
   ...
   def jsonrpc_gettxout(self, txHash, n, binary=0):
      ...
      RETURN:
      A dictionary with the Peercoin amount for the TxOut and the TxOut script in
   ...
   def jsonrpc_dumpprivkey(self, addr58):
      ...
      if not checkAddrStrValid(addr58):
         raise InvalidPeercoinAddress

      addr160 = addrStr_to_hash160(addr58, False)[1]

      pyPPCAddress = self.curWlt.getAddrByHash160(addr160)
      if pyPPCAddress == None:
         raise PrivateKeyNotFound

      return binary_to_hex(pyPPCAddress.serializePlainPrivateKey())
   ...
   def jsonrpc_getbalance(self, baltype='spendable'):
      ...
      RETURN:
      The current wallet balance (PPC), or -1 if an error occurred.
      """
   ...
   def jsonrpc_getreceivedbyaddress(self, address):
      ...
      RETURN:
      The balance received from the incoming address (PPC).
      """
   ...
   def jsonrpc_createustxtoaddress(self, recAddr, amount):
      ...
      amount - The number of Peercoins to send to the recipient.
      RETURN:
   ...
   #
   # Example: We wish to send 1 PPC to a lockbox and 0.12 PPC to a standard
   # Peercoin address. (Publick keys and P2SH scripts can also be specified as
   ...
   def jsonrpc_createustxformany(self, *args):
      ...
      args - An indefinite number of comma-separated sets of recipients and the
             number of Peercoins to send to the recipients. The recipients can be
   ...
   def jsonrpc_getledger(self, inB58ID, tx_count=10, from_tx=0, simple=False):
      ...
      if ledgerWlt == None:
         ...
         for i in range(lower,upper):
            ...
            # Get some more data.
            # amtCoins: amt of PPC transacted, always positive (how big are
   ...
   #############################################################################
   # A semi-analogue to peercoind's getinfo().
   @catchErrsForJSON
   def jsonrpc_getarmorydinfo(self):
      ...
      info = { \
               'versionstr':        getVersionString(PPCARMORY_VERSION),
               'version':           getVersionInt(PPCARMORY_VERSION),
               #'protocolversion':   0,
               'walletversionstr':  getVersionString(PYPPCWALLET_VERSION),
               'walletversion':     getVersionInt(PYPPCWALLET_VERSION),
   ...
   # https://peercointalk.org/index.php?topic=92496.msg1126310#msg1126310
   ...
   # SendPeercoinsFrame::validateInputsGetUSTX() (ui/TxFrames.py).
...
class Armory_Daemon(object):
   def __init__(self, wlt=None, lb=None):
      ...
      # WltMap:   wltID --> PyPPCWallet
      self.WltMap = {}
      self.wltIDSet = set()

      # lboxMap:           lboxID --> MultiSigLockbox
      # lboxCppWalletMap:  lboxID --> Cpp.PPCWallet
      self.lboxMap = {}   
      ...
      if armorydIsRunning == True:
         ...
         if setupNetworking():
            ...
            LOGWARN('* Please note that armoryd v%s is beta software and is still in ' % \
                  getVersionString(PPCARMORY_VERSION))
            LOGWARN('* development. Whenever applicable, the interface is designed to match ')
            LOGWARN('* that of peercoind, with function parameters and return values closely ')
            LOGWARN('* matching those of peercoind. Despite this, the function parameters and ')
            LOGWARN('* return values may change, both for ported peercoind function and ')
            ...
            if lb:
               ...
               if len(self.lboxMap) > 0:
                  ...
                  for lbID,lbox in self.lboxMap.iteritems():
                     self.lboxCppWalletMap[lbID] = PPCWallet()
            ...
            errStr = 'armoryd is not ready to run! Please check to see if ' \
                     'peercoind is running and the Blockchain files ' \
      ...
      if(not TheBDM.getBDMState()=='Offline'):
         ...
         if len(vectMissingBlks) > 0:
            LOGERROR('Armory has detected an error in the blockchain ' \
                     ...
                     'software (Peercoin-Qt or peercoind). This error is not ' \
         ...
         # This is CONNECT call for armoryd to talk to peercoind
         LOGINFO('Set up connection to peercoind')
   ...
   def checkWallet(self):
      if self.lock.acquire(False):
         wltStatus = PyPPCWalletRecovery().ProcessWallet(None, self.curWlt, \
                                                         Mode=5)
         if wltStatus != 0:
